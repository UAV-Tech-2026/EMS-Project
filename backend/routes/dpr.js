...
function isAllowedDate(dprDate) {
  ...
  return dprDate === today || dprDate === tomorrow;
}

// Helper: safely format time value from DB (Date object or HH:MM string)
...
// ─── EMPLOYEE: Get today's EOD tasks for dashboard ───────────────────────────
...
// ─── DOWNLOAD DPR EXCEL ───────────────────────────────────────────────────────
// GET /dpr/download?date=2026-03-26
...
router.get("/all", verifyToken, async (req, res) => {
  try {
    ...
    const result   = await pool.query(`
      ...
      JOIN employees e ON u.id = e.user_id
      ${date ? "WHERE d.dpr_date = $1" : ""}
      ORDER BY d.dpr_date DESC, u.fullname ASC
    `, date ? [date] : []);
...
// ─── SUPER ADMIN: export whole project DPR in excel ──────────────────────────
router.get("/download-all", verifyToken, async (req, res) => {
  try {
    ...
    const params = [];
    if (date) {
      query += " AND d.dpr_date = $1";
      params.push(date);
    } else if (start_date && end_date) {
      query += " AND d.dpr_date >= $1 AND d.dpr_date <= $2";
...
router.get("/tasks/:userId", verifyToken, async (req, res) => {
  try {
    if (req.user.role !== "super_admin" && req.user.role !== "admin")
      return res.status(403).json({ msg: "Access denied" });
