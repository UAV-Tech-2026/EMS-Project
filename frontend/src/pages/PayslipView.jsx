...
export default function PayslipView() {
  ...
  const fetchYtd = async (uid) => {
    try {
      ...
      setYtdData(res.data || null);
    } catch (err) {
      console.error("YTD Fetch failed", err);
      setYtdData(null); // keep null so UI skips the block safely
    }
  };
  ...
  // Safe YTD values — handles missing cumulative or missing fields
  const ytd = ytdData?.cumulative ?? null;
    ...
    <div className="leave-container" style={{ padding: "2rem", maxWidth: "1000px", margin: "0 auto" }}>
      <div className="leave-card" style={{ background: "#fff", padding: "2rem", borderRadius: "12px", boxShadow: "0 4px 6px -1px rgba(0,0,0,0.1)" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
            <div style={{
              width: 42, height: 42, background: "#ffffff", borderRadius: 10,
        ...
        {/* ── YTD Summary — only shown when cumulative data exists ── */}
        {ytd && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1.5rem", marginBottom: "2rem" }}>
            <div style={{ background: "linear-gradient(135deg, #ebf8ff 0%, #bee3f8 100%)", padding: "1.5rem", borderRadius: "12px", border: "1px solid #90cdf4" }}>
              ...
              <div style={{ color: "#2a4365", fontSize: "1.5rem", fontWeight: 800 }}>₹{(ytd.ytd_gross ?? 0).toLocaleString("en-IN")}</div>
            </div>
            <div style={{ background: "linear-gradient(135deg, #fff5f5 0%, #fed7d7 100%)", padding: "1.5rem", borderRadius: "12px", border: "1px solid #feb2b2" }}>
              ...
              <div style={{ color: "#742a2a", fontSize: "1.5rem", fontWeight: 800 }}>₹{(ytd.ytd_deductions ?? 0).toLocaleString("en-IN")}</div>
            </div>
            <div style={{ background: "linear-gradient(135deg, #f0fff4 0%, #c6f6d5 100%)", padding: "1.5rem", borderRadius: "12px", border: "1px solid #9ae6b4" }}>
              ...
              <div style={{ color: "#234e33", fontSize: "1.5rem", fontWeight: 800 }}>₹{(ytd.ytd_net ?? 0).toLocaleString("en-IN")}</div>
            </div>
          </div>
        )}
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
            ...
            <tbody>
              {requests.map((r, index) => {
                ...
                return (
                  <tr key={r.id ?? index} style={{ borderBottom: "1px solid #edf2f7" }}>
      ...
      <div style={{ marginTop: "2rem", display: "flex", justifyContent: "flex-end" }}>
        <button
          ...
          style={{ background: "#fff", color: "#475569", border: "1px solid #e2e8f0", padding: "10px 24px", borderRadius: "10px", fontWeight: "700", fontSize: "14px", cursor: "pointer", boxShadow: "0 2px 4px rgba(0,0,0,0.02)", display: "flex", alignItems: "center", gap: "8px" }}
        >
          ← Back to Dashboard
        </button>
      </div>
