...
app.get("/health", (req, res) => {
  res.json({ service: "maintenance-service", status: "UP", port: process.env.PORT });
});

// Feature route: POST /maintenance
app.post("/maintenance", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "POST /maintenance successful in maintenance-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: GET /maintenance/:productId
app.get("/maintenance/:productId", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "GET /maintenance/:productId successful in maintenance-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
