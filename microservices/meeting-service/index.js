...
app.get("/health", (req, res) => {
  res.json({ service: "meeting-service", status: "UP", port: process.env.PORT });
});

// Feature route: GET /meetings
app.get("/meetings", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "GET /meetings successful in meeting-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: POST /meetings
app.post("/meetings", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "POST /meetings successful in meeting-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: PUT /meetings/:id
app.put("/meetings/:id", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "PUT /meetings/:id successful in meeting-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: DELETE /meetings/:id
app.delete("/meetings/:id", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "DELETE /meetings/:id successful in meeting-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
...
app.listen(PORT, () => {
  console.log("meeting-service is running on port " + PORT);
});
