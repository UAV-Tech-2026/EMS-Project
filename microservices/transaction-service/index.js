...
app.get("/health", (req, res) => {
  res.json({ service: "transaction-service", status: "UP", port: process.env.PORT });
});

// Feature route: POST /stock-out
app.post("/stock-out", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "POST /stock-out successful in transaction-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: POST /return
app.post("/return", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "POST /return successful in transaction-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: GET /reports/transactions/:productId
app.get("/reports/transactions/:productId", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "GET /reports/transactions/:productId successful in transaction-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
