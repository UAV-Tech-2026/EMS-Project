...
app.get("/health", (req, res) => {
  res.json({ service: "withdrawal-service", status: "UP", port: process.env.PORT });
});

// Feature route: GET /my-withdrawals
app.get("/my-withdrawals", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "GET /my-withdrawals successful in withdrawal-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: GET /all-withdrawals
app.get("/all-withdrawals", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "GET /all-withdrawals successful in withdrawal-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: GET /reports/withdrawal-summary/:productId
app.get("/reports/withdrawal-summary/:productId", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "GET /reports/withdrawal-summary/:productId successful in withdrawal-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
...
app.listen(PORT, () => {
  console.log("withdrawal-service is running on port " + PORT);
});
