...
app.get("/health", (req, res) => {
  res.json({ service: "leave-service", status: "UP", port: process.env.PORT });
});

// Feature route: GET /leaves
app.get("/leaves", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "GET /leaves successful in leave-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: POST /leaves/apply
app.post("/leaves/apply", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "POST /leaves/apply successful in leave-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: PUT /leaves/:id/approve
app.put("/leaves/:id/approve", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "PUT /leaves/:id/approve successful in leave-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: PUT /leaves/:id/reject
app.put("/leaves/:id/reject", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "PUT /leaves/:id/reject successful in leave-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: GET /leaves/my
app.get("/leaves/my", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "GET /leaves/my successful in leave-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
