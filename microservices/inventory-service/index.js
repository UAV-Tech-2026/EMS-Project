...
app.get("/health", (req, res) => {
  res.json({ service: "inventory-service", status: "UP", port: process.env.PORT });
});

// Feature route: GET /products
app.get("/products", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "GET /products successful in inventory-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: POST /products
app.post("/products", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "POST /products successful in inventory-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: GET /products/:id
app.get("/products/:id", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "GET /products/:id successful in inventory-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: POST /products/:id/add-quantity
app.post("/products/:id/add-quantity", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "POST /products/:id/add-quantity successful in inventory-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: POST /stock-in
app.post("/stock-in", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "POST /stock-in successful in inventory-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
