...
app.get("/health", (req, res) => {
  res.json({ service: "auth-service", status: "UP", port: process.env.PORT });
});

// Feature route: POST /auth/login
app.post("/auth/login", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "POST /auth/login successful in auth-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: GET /auth/me
app.get("/auth/me", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "GET /auth/me successful in auth-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: GET /permissions/my
app.get("/permissions/my", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "GET /permissions/my successful in auth-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: PUT /permissions/:userId
app.put("/permissions/:userId", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "PUT /permissions/:userId successful in auth-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
});
// Feature route: GET /admins
app.get("/admins", authMiddleware, async (req, res) => {
  try {
    // Add feature logic here
    res.json({ message: "GET /admins successful in auth-service" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
